...
/**
  ...
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_ShowFloatNum(int16_t X, int16_t Y, double Number, uint8_t IntLength, uint8_t FraLength, uint8_t FontSize ,bool sign)
{
	uint32_t PowNum, IntNum, FraNum;
	
	if (Number >= 0 && sign)						//数字大于等于0
	{
		OLED_ShowChar(X, Y, '+', FontSize);	//显示+号
	}
	else if(Number <= 0 && sign)									//数字小于0
	{
		OLED_ShowChar(X, Y, '-', FontSize);	//显示-号
